...
def clean_stock(stock_file):
    ...
    avg_vol_per_week = stock.groupBy("week").agg(avg(col("Volume")).alias("avgVol"))
    stock = stock.join(avg_vol_per_week, on=["week"], how="inner")
    stock = stock.withColumn("aboveAvgVol", when(col("Volume") > col("avgVol"), 1).otherwise(0))
    ...
    stock = stock.groupBy("week", "year", "startDate", "endDate").agg(first("Close").alias("startClose"), last("Close").alias("endClose"), first("aboveAvgVol").alias("aboveAvgVol"))

    stock = stock.withColumn("highChange", when((col("endClose") - col("startClose")) / col("startClose") > 0.06, 1).otherwise(0))
    stock = stock.select("week", "year", "startDate", "endDate", "endClose", "aboveAvgVol", "highChange").withColumnRenamed("endClose", "Close")
